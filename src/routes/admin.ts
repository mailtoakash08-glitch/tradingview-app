...
router.post("/reset-auto-stop", (req: Request, res: Response) => {
  ...
  res.status(200).json({
    status: "ok",
    message:
      "Auto-stop has been reset. Trading can resume if kill-switch is also disabled.",
...
router.get("/broker-status", (req: Request, res: Response) => {
  const status = brokerRouter.getBrokerStatus();
  ...
  res.status(200).json({
    ...
    brokers: {
      ...
      lightspeed: {
        connected: status.lightspeed,
        status: status.lightspeed ? "Connected" : "Disconnected",
      },
    },
    timestamp: new Date().toISOString(),
  });
